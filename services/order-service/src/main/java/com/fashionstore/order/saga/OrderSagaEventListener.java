...
public class OrderSagaEventListener {
    ...
    /**
     * Khớp theo variantId + số lượng đã đặt, không theo cartItemId đã snapshot: khách xoá rồi thêm lại một
     * sản phẩm là id đổi (cách cũ bỏ sót dòng đó), còn khách tăng số lượng sau khi checkout thì phần dư
     * phải ở lại giỏ (cách cũ xoá cả dòng). Sản phẩm khách thêm sau checkout không nằm trong đơn nên không
     * bị chạm tới.
     */
    private void removeOrderedCartItems(Order order) {
        Cart cart = cartRepository.findByUserIdAndStatus(order.getUserId(), CartStatus.ACTIVE).orElse(null);
        if (cart == null) {
            return;
        }

        Map<String, Integer> orderedQuantities = order.getItems().stream()
                .collect(Collectors.toMap(OrderItem::getVariantId, OrderItem::getQuantity, Integer::sum));

        cart.getItems().removeIf(item -> {
            Integer ordered = orderedQuantities.get(item.getVariantId());
            if (ordered == null) {
                return false;
            }
            if (item.getQuantity() <= ordered) {
                return true;
            }
            item.setQuantity(item.getQuantity() - ordered);
            return false;
        });
